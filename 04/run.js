...
lineReader.on("close", function () {
  ...
  input.forEach(function (line) {
    ...
    if (
      checkFullOverlap(leftOrder, rightOrder) ||
      checkFullOverlap(rightOrder, leftOrder)
    ) {
      sum += 1;
    }
    if (
      checkPartialOverlap(leftOrder, rightOrder) ||
      checkPartialOverlap(rightOrder, leftOrder)
    )
      sum2 += 1;
  });
